...
app_ui = ui.page_fluid(
    ...
    ui.div(
        ...
        ui.div(
            ...
            ui.p(
                "Prix = 10 € × probabilité de survie des deux barrières × actualisation. Monitoring simulé à chaque séance, drift risk-neutral r−q.",
                class_="small-muted",
            ),
        ),
        ui.div(
...
def server(input, output, session):
    ...
    def product_table():
        ...
        out["Prix marché"] = out["Prix_marche"].map(lambda x: f"{x:.2f} €")
        out["Edge"] = out["Edge_theo_moins_marche"].map(lambda x: f"{x:+.3f} €")
