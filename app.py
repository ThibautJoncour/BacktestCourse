...
APP_TITLE = "Signal Stability + Pricing NVDA"
...
app_ui = ui.page_fluid(
    ...
    ui.div(
        ...
        ui.div(
            ...
            ui.h1(APP_TITLE),
            ui.p(
                "Le modèle Ensemble calcule le signal SPX, le Jump Model indique le régime Bull/Bear, les modèles HAR prévoient la RV/VIX/VRP, et un HAR-RV NVIDIA alimente le Monte Carlo des Stability NVDA."
            ),
            ui.div(
                {"class": "refresh-row"},
        ...
        ui.div(
            ...
            ui.p(
                "Chaque prévision est annualisée. Pour les Stability dont la maturité résiduelle dépasse 5 séances, le Monte Carlo utilise la prévision HAR-RV J+5 comme proxy de sigma constant sur la vie résiduelle.",
                class_="small-muted",
            ),
        ),
        ui.div(
            ...
            ui.p(
                "La liste des Stability NVIDIA est récupérée automatiquement depuis SG Bourse à chaque actualisation. Prix = 10 € × probabilité de survie des deux barrières × actualisation. Monitoring simulé à chaque séance, drift risk-neutral r−q.",
                class_="small-muted",
            ),
        ),
        ui.div(
...
def server(input, output, session):
    ...
    def nvda_har_table():
        ...
        table = r.get("nvda_har_rv_table")
        if table is None or len(table) == 0:
            return pd.DataFrame({"Mesure": ["HAR-RV NVDA"], "Valeur": ["Indisponible"]})

        out = table.copy()
        out["Horizon"] = out["Horizon_jours"].map(lambda x: f"J+{int(x)}")
        out["RV HAR annualisée"] = out["HAR_RV_pred_pct_ann"].map(lambda x: f"{x:.2f} %")
        out["R² log (in-sample)"] = out["R2_log_in_sample"].map(lambda x: f"{x:.3f}")
        return out[["Horizon", "RV HAR annualisée", "Train_rows", "R² log (in-sample)"]].rename(
            columns={"Train_rows": "Observations train"}
        )
    ...
    def product_table():
        ...
        table = r.get("nvda_stability_mc")
        if table is None or len(table) == 0:
            return pd.DataFrame(
                {"Information": ["Monte Carlo NVDA"], "Valeur": ["Indisponible"]}
            )

        out = table.copy()

        # Classement des Stability par edge croissant (valeur numérique).
        # Le tri est effectué avant le formatage en chaîne pour éviter un tri lexicographique.
        out = out.sort_values(
            by="Edge_theo_moins_marche", ascending=True, na_position="last"
        ).reset_index(drop=True)

        out["Barrières"] = out.apply(
            lambda row: f"{row['Barriere_basse']:.0f} / {row['Barriere_haute']:.0f}", axis=1
        )
        out["Sigma HAR"] = out["Sigma_HAR_pct_ann"].map(lambda x: f"{x:.2f} %")
        out["Survie MC"] = out["Prob_survie_MC"].map(lambda x: f"{100*x:.2f} %")
        out["Prix théorique"] = out["Prix_theorique_MC"].map(lambda x: f"{x:.3f} €")
        out["Prix marché"] = out["Prix_marche"].map(
            lambda x: "—" if pd.isna(x) else f"{x:.2f} €"
        )
        out["Edge"] = out["Edge_theo_moins_marche"].map(
            lambda x: "—" if pd.isna(x) else f"{x:+.3f} €"
        )
        out["SE MC"] = out["Erreur_std_MC"].map(lambda x: f"{x:.4f} €")
        out["Source prix"] = out.get("Source_prix", pd.Series("—", index=out.index)).fillna("—")
