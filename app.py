...
def server(input, output, session):
    ...
    def nvda_har_table():
        ...
        out["MAE OOS"] = out["MAE_vol_pct"].map(lambda x: f"{x:.2f} pts")
        out["RMSE OOS"] = out["RMSE_vol_pct"].map(lambda x: f"{x:.2f} pts")
        out["QLIKE OOS"] = out["QLIKE"].map(lambda x: f"{x:.3f}")
        out["Biais OOS"] = out["Bias_vol_pct"].map(lambda x: f"{x:+.2f} pts")
        return out[[
            "Horizon", "RV HAR annualisée", "OOS_n", "MAE OOS",
            "RMSE OOS", "QLIKE OOS", "Biais OOS"
        ]].rename(columns={"OOS_n": "Observations OOS"})
    ...
    def product_table():
        ...
        # Classement des Stability par edge décroissant (valeur numérique).
        # Le tri est effectué avant le formatage en chaîne pour éviter un tri lexicographique.
        out = out.sort_values(
            by="Edge_theo_moins_marche", ascending=False, na_position="last"
        ...
        out["Sigma LT"] = out["Sigma_long_run_pct_ann"].map(lambda x: f"{x:.2f} %")
