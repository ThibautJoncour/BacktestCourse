...
CODE_VERSION = "NVDA_HAR_RV_MC_STABILITY_SG_AUTO_2026-09-09"
...
def fetch_nvda_stability_products() -> pd.DataFrame:
    ...
    if not rows:
        details = errors[-1] if errors else "aucune ligne HTML reconnue"
        raise StrategyError(
            "Impossible de récupérer les Stability NVIDIA depuis SG Bourse. "
            f"Dernier détail: {details}"
        )
...
def _price_nvda_stabilities_mc(
    ...
    for maturity, group in sorted(grouped.items()):
        ...
        for product in group:
            ...
            edge = (
                theo_price - market_price_float
                if np.isfinite(market_price_float)
                else np.nan
            )
            rows.append({
                "Code": code,
