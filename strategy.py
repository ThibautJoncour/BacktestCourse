...
CODE_VERSION = "VIX3M_FIX_V5_WITH_HAR_VRP_GARCH_2026-08-05"
...
# ============================================================
# HAR-RV / HAR-VIX À 5 JOURS ET COMPRESSION GARCH
# ============================================================

HAR_HORIZON = 5
HAR_START_TRAINING = 100
HAR_HISTORY_DAYS = 729
HAR_INTERVAL = "1h"
TRADING_DAYS = 252
GARCH_COMPRESSION_QUANTILE = 0.10
GARCH_MIN_OBS = 80


def _download_hourly_close(ticker: str, name: str) -> pd.Series:
    end = pd.Timestamp.utcnow().tz_localize(None).normalize() + pd.Timedelta(days=1)
    start = end - pd.Timedelta(days=HAR_HISTORY_DAYS)
    data = yf.download(
        ticker,
        start=start,
        end=end,
        interval=HAR_INTERVAL,
        auto_adjust=False,
        progress=False,
        prepost=False,
        threads=False,
    ...
    if data.empty:
        raise StrategyError(f"Aucune donnée horaire reçue pour {ticker}.")
    close = data["Close"]
    if isinstance(close, pd.DataFrame):
        close = close.iloc[:, 0]
    close = pd.to_numeric(close, errors="coerce").rename(name)
    close.index = pd.to_datetime(close.index, utc=True).tz_convert("America/New_York").tz_localize(None)
    return close.dropna().sort_index()


def _future_mean(series: pd.Series, horizon: int) -> pd.Series:
    return (
        series.shift(-1).iloc[::-1]
        .rolling(horizon, min_periods=horizon)
        .mean().iloc[::-1]
    )


def _build_har_intraday_dataset() -> pd.DataFrame:
    spx = _download_hourly_close("^GSPC", "spx")
    vix = _download_hourly_close("^VIX", "vix")

    spx_df = spx.to_frame()
    spx_df["Date"] = spx_df.index.normalize()
    spx_df["ret"] = np.log(spx_df["spx"] / spx_df["spx"].shift(1))
    spx_df.loc[spx_df["Date"] != spx_df["Date"].shift(1), "ret"] = np.nan
    spx_df["r2"] = spx_df["ret"] ** 2
    rv = spx_df.groupby("Date").agg(RV=("r2", "sum"), n=("r2", "count"))
    rv = rv.loc[rv["n"] >= 4].copy()
    rv = rv.loc[rv["RV"] > 0].copy()

    vix_df = vix.to_frame()
    vix_df["Date"] = vix_df.index.normalize()
    vd = vix_df.groupby("Date").agg(VIX=("vix", "last"), n_vix=("vix", "count"))
    vd = vd.loc[vd["n_vix"] >= 4].copy()

    data = rv.join(vd[["VIX"]], how="inner").sort_index()
    data["IV_current"] = (data["VIX"] / 100.0) ** 2 / TRADING_DAYS

    data["logRV_d"] = np.log(data["RV"] + 1e-12)
    data["RV_5"] = data["RV"].rolling(5, min_periods=5).mean()
    data["RV_22"] = data["RV"].rolling(22, min_periods=22).mean()
    data["logRV_w"] = np.log(data["RV_5"] + 1e-12)
    data["logRV_m"] = np.log(data["RV_22"] + 1e-12)

    data["logVIX_d"] = np.log(data["VIX"] + 1e-12)
    data["VIX_5"] = data["VIX"].rolling(5, min_periods=5).mean()
    data["VIX_22"] = data["VIX"].rolling(22, min_periods=22).mean()
    data["logVIX_w"] = np.log(data["VIX_5"] + 1e-12)
    data["logVIX_m"] = np.log(data["VIX_22"] + 1e-12)
    data["dlogVIX"] = data["logVIX_d"].diff()

    data["RV_target_5d"] = _future_mean(data["RV"], HAR_HORIZON)
    data["VIX_target_5d"] = _future_mean(data["VIX"], HAR_HORIZON)
    data["IV_target_5d"] = _future_mean(data["IV_current"], HAR_HORIZON)
    data["log_RV_target_5d"] = np.log(data["RV_target_5d"] + 1e-12)
    data["log_VIX_target_5d"] = np.log(data["VIX_target_5d"] + 1e-12)
    return data.replace([np.inf, -np.inf], np.nan)


def _garch_compression_status(dlog_vix: pd.Series) -> dict:
    clean = dlog_vix.dropna().astype(float)
    if arch_model is None or len(clean) < GARCH_MIN_OBS:
        return {
            "garch_sigma": np.nan,
            "garch_compression_threshold": np.nan,
            "garch_compression": False,
        }
    scaled = clean * 100.0
    try:
        model = arch_model(
            scaled, mean="Constant", vol="GARCH", p=1, q=1, dist="t", rescale=False
        )
        fit = model.fit(disp="off", show_warning=False)
        conditional_sigma = fit.conditional_volatility / 100.0
        current_sigma = float(conditional_sigma.iloc[-1])
        historical = conditional_sigma.iloc[:-1].dropna()
        threshold = float(historical.quantile(GARCH_COMPRESSION_QUANTILE))
        return {
            "garch_sigma": current_sigma,
            "garch_compression_threshold": threshold,
            "garch_compression": bool(current_sigma <= threshold),
        }
    except Exception:
        return {
            "garch_sigma": np.nan,
...
def compute_har_vrp_forecast() -> dict:
    data = _build_har_intraday_dataset()
    rv_features = ["logRV_d", "logRV_w", "logRV_m"]
    vix_features = ["logVIX_d", "logVIX_w", "logVIX_m"]
    valid = data.dropna(subset=rv_features + vix_features + ["RV_22", "VIX_22"]).copy()
    if len(valid) < HAR_START_TRAINING + HAR_HORIZON + 5:
        raise StrategyError("Historique insuffisant pour HAR-RV / HAR-VIX.")

    rows = []
    for i in range(len(valid)):
        train_end = i - HAR_HORIZON + 1
        if train_end <= 0:
            continue
        train = valid.iloc[:train_end].dropna(
            subset=rv_features + vix_features + ["log_RV_target_5d", "log_VIX_target_5d"]
        )
        test = valid.iloc[[i]]
        if len(train) < HAR_START_TRAINING:
            continue
        if test[["RV_target_5d", "VIX_target_5d", "IV_target_5d"]].isna().any().any():
            continue
        m_rv = LinearRegression().fit(train[rv_features], train["log_RV_target_5d"])
        m_vix = LinearRegression().fit(train[vix_features], train["log_VIX_target_5d"])
        pred_rv = float(np.exp(m_rv.predict(test[rv_features])[0]))
        pred_vix = float(np.exp(m_vix.predict(test[vix_features])[0]))
        pred_iv = (pred_vix / 100.0) ** 2 / TRADING_DAYS
        current_iv = float(test["IV_current"].iloc[0])
        real_rv = float(test["RV_target_5d"].iloc[0])
        real_iv = float(test["IV_target_5d"].iloc[0])
        rows.append({
            "Date": test.index[-1],
            "VRP_tradable_pred_points": (current_iv - pred_rv) * TRADING_DAYS * 10000.0,
            "VRP_tradable_real_points": (current_iv - real_rv) * TRADING_DAYS * 10000.0,
            "VRP_future_pred_points": (pred_iv - pred_rv) * TRADING_DAYS * 10000.0,
            "VRP_future_real_points": (real_iv - real_rv) * TRADING_DAYS * 10000.0,
        })

    history = pd.DataFrame(rows)

    latest = valid.iloc[[-1]]
    train = valid.iloc[: len(valid) - HAR_HORIZON + 1].dropna(
        subset=rv_features + vix_features + ["log_RV_target_5d", "log_VIX_target_5d"]
    )
    m_rv = LinearRegression().fit(train[rv_features], train["log_RV_target_5d"])
    m_vix = LinearRegression().fit(train[vix_features], train["log_VIX_target_5d"])
    pred_rv = float(np.exp(m_rv.predict(latest[rv_features])[0]))
    pred_vix = float(np.exp(m_vix.predict(latest[vix_features])[0]))
    pred_iv = (pred_vix / 100.0) ** 2 / TRADING_DAYS
    current_iv = float(latest["IV_current"].iloc[0])
    garch = _garch_compression_status(valid["dlogVIX"])

    return {
        "har_date": latest.index[-1],
        "iv_current_vol_pct": float(latest["VIX"].iloc[0]),
        "rv_forecast_vol_pct": float(np.sqrt(pred_rv * TRADING_DAYS) * 100.0),
        "vix_forecast_5d": pred_vix,
        "vrp_current_iv_forecast_points": (current_iv - pred_rv) * TRADING_DAYS * 10000.0,
        "vrp_future_forecast_points": (pred_iv - pred_rv) * TRADING_DAYS * 10000.0,
        "vrp_history": history.tail(250),
        **garch,
    }
...
def compute_daily_signal() -> dict:
    ...
    har_vrp = compute_har_vrp_forecast()
