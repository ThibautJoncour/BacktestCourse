...
CODE_VERSION = "NVDA_HAR_RV_MC_STABILITY_2026-09-09"
...
# code, barrière basse, barrière haute, maturité, prix marché
NVDA_STABILITY_PRODUCTS = [
    ("8E45S", 180.0, 270.0, "2026-09-18", 9.80),
    ("7C50S", 100.0, 240.0, "2026-09-18", 7.50),
    ("7C52S", 120.0, 260.0, "2026-09-18", 9.84),
    ("7C54S", 160.0, 280.0, "2026-09-18", 9.90),
    ("7C57S", 140.0, 280.0, "2026-10-16", 9.52),
    ("7C55S", 100.0, 280.0, "2026-10-16", 9.60),
    ("2R74S", 160.0, 260.0, "2026-10-16", 8.29),
    ("2R79S", 140.0, 260.0, "2026-11-20", 6.13),
    ("2R83S", 160.0, 280.0, "2026-11-20", 7.73),
    ("2R85S", 160.0, 320.0, "2026-11-20", 9.04),
    ("2R80S", 140.0, 280.0, "2026-11-20", 8.07),
    ("2R82S", 140.0, 320.0, "2026-11-20", 9.39),
    ("2R78S", 120.0, 320.0, "2026-11-20", 9.51),
    ("2R84S", 160.0, 300.0, "2026-11-20", 8.66),
    ("2R81S", 140.0, 300.0, "2026-11-20", 9.00),
    ("2R77S", 120.0, 300.0, "2026-11-20", 9.11),
    ("2R75S", 120.0, 260.0, "2026-11-20", 6.25),
]
...
def _download_nvidia_history() -> pd.DataFrame:
    """Télécharge NVDA et construit les briques de volatilité du HAR-RV."""
    nvda = _download_close(NVDA_TICKER, "nvda")
    out = pd.DataFrame({"nvda": nvda})
    out["return_1d"] = np.log(out["nvda"] / out["nvda"].shift(1))

    # Proxy HAR sur données quotidiennes : vol annualisée 1j / 5j / 21j.
    out["rv_d"] = out["return_1d"].abs() * np.sqrt(252) * 100
    out["rv_w"] = np.sqrt(out["return_1d"].pow(2).rolling(5).mean() * 252) * 100
    out["rv_m"] = np.sqrt(out["return_1d"].pow(2).rolling(21).mean() * 252) * 100
    return out.replace([np.inf, -np.inf], np.nan)


def _har_rv_forecast_horizons_nvda(nvda: pd.DataFrame, max_horizon: int = 5) -> dict:
    """
    Prévisions HAR-RV NVDA de J+1 à J+5.

    Un Ridge séparé est estimé pour chaque horizon h. La cible est la
    volatilité réalisée annualisée sur les h prochaines séances.
    Les régressions sont faites en log-vol pour garantir une prévision > 0.
    """
    predictors = nvda[["rv_d", "rv_w", "rv_m"]].copy()
    latest = predictors.dropna().iloc[[-1]]
    if latest.empty:
        raise StrategyError("Aucune observation récente exploitable pour HAR-RV NVDA.")
    ...
    for h in range(1, max_horizon + 1):
        # Somme des carrés des rendements t+1 ... t+h, puis annualisation.
        future_var = (
            nvda["return_1d"].pow(2)
            .shift(-1)
            .rolling(h)
            .sum()
            .shift(-(h - 1))
        )
        target = np.sqrt(future_var * 252 / h) * 100
        ...
        if len(train) < 250:
            raise StrategyError(
                f"Historique insuffisant pour HAR-RV NVDA horizon {h}j : {len(train)} lignes."
            )
        ...
        X = np.log(train[["rv_d", "rv_w", "rv_m"]].clip(lower=1e-6))
        y = np.log(train["target"].clip(lower=1e-6))
        model.fit(X, y)
        pred = float(np.exp(model.predict(np.log(latest.clip(lower=1e-6)))[0]))

        forecasts[h] = pred
        train_rows[h] = int(len(train))
        r2_in_sample[h] = float(model.score(X, y))

    table = pd.DataFrame({
        "Horizon_jours": list(forecasts.keys()),
        "HAR_RV_pred_pct_ann": list(forecasts.values()),
        "Train_rows": [train_rows[h] for h in forecasts],
        "R2_log_in_sample": [r2_in_sample[h] for h in forecasts],
    })
...
def _price_nvda_stabilities_mc(
    ...
    products=NVDA_STABILITY_PRODUCTS,
    ...
    """
    Prix MC des Stability NVDA.

    sigma n'est plus fixé arbitrairement : on utilise la HAR-RV annualisée.
    Pour une maturité > 5 séances, la prévision HAR à 5 jours est utilisée
    comme proxy de volatilité annualisée constante sur la vie résiduelle.
    Pour une maturité <= 5 séances, on prend l'horizon HAR correspondant.

    Le drift est risk-neutral (r-q). Les barrières sont observées à chaque
    séance simulée (monitoring discret journalier).
    """
    if spot <= 0:
    ...
    grouped = {}
    for code, low, high, maturity, market_price in products:
        maturity = pd.Timestamp(maturity).normalize()
        grouped.setdefault(maturity, []).append((code, low, high, market_price))

    for maturity, group in sorted(grouped.items()):
        t_days = _business_days_to_maturity(today, maturity)
        sigma_h = min(max(t_days, 1), 5)
        sigma_pct = float(har_forecasts[sigma_h])
        sigma = sigma_pct / 100.0

        # Simule une seule fois par maturité et réutilise les mêmes chemins
        # pour tous les couples de barrières de cette maturité.
        ...
        for code, low, high, market_price in group:
            if not (low < spot < high):
                ...
                # Erreur standard MC du prix d'un Bernoulli * payoff actualisé.
                mc_se = float(
                    NVDA_REDEMPTION * discount
                    * np.sqrt(max(survival * (1.0 - survival), 0.0) / nb_paths)
                )

            edge = theo_price - float(market_price)
            rows.append({
                "Code": code,
                "Spot": float(spot),
                ...
                "Sigma_HAR_horizon_j": int(sigma_h),
                "Sigma_HAR_pct_ann": sigma_pct,
                "Prob_survie_MC": survival,
                "Prix_theorique_MC": theo_price,
                "Erreur_std_MC": mc_se,
                "Prix_marche": float(market_price),
